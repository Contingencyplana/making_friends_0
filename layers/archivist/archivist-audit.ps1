...
foreach ($f in $md) {
  ...
  # Must start with front-matter at the very top (no blank line before ---)
  if ($t -notmatch '^(?s)---\r?\n.*?\r?\n---\s*\r?\n') {
  ...
  # Exactly one H1 — ignore fenced code blocks
  $bodyForH1 = $t `
    -replace '(?s)```.*?```','' `
    -replace '(?s)~~~.*?~~~',''
...
# B) Binary sidecars (skip dist/)
$binExt = @(".png",".jpg",".jpeg",".gif",".webp",".mp3",".wav",".mp4",".mov")
$bin = Get-ChildItem -Path $Root -Recurse -File |
  Where-Object { $binExt -contains $_.Extension.ToLower() -and $_.FullName -notmatch "\\dist\\" }

foreach ($b in $bin) {
  $meta = "$($b.FullName).meta.json"
  if (-not (Test-Path $meta)) {
    Add-Issue $b.FullName "Missing sidecar (.meta.json)"
  }
}
